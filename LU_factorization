% =========================================================================
% Generalized Newton-Raphson Power Flow using MATPOWER Data Format
% Linear Solver: Custom LU Decomposition
% =========================================================================
clear; clc;

%% 1. Load and Parse MATPOWER Data
% Call the case30 function (ensure case30.m is in your current folder)
mpc = case30();
baseMVA = mpc.baseMVA;
nbus = size(mpc.bus, 1);
...
% Extract Bus Data
bus_id = mpc.bus(:, 1);
bus_type = mpc.bus(:, 2); % 1=PQ, 2=PV, 3=Slack
V = mpc.bus(:, 8);        % Initial Voltage Magnitude Guess
delta = mpc.bus(:, 9) * pi / 180; % Initial Voltage Angle (converted to radians)

% Extract Loads and Shunts (Convert to per-unit)
Pd = mpc.bus(:, 3) / baseMVA;
Qd = mpc.bus(:, 4) / baseMVA;
Gs = mpc.bus(:, 5) / baseMVA;
Bs = mpc.bus(:, 6) / baseMVA;

% Extract Generation (Map generators to their respective buses)
Pg = zeros(nbus, 1);
Qg = zeros(nbus, 1);
for i = 1:size(mpc.gen, 1)
    ...
    % Enforce generator voltage setpoints for PV and Slack buses
...
non_slack = [pv; pq]; % All buses where active power is specified
num_non_slack = length(non_slack);
num_pq = length(pq);

%% 2. Formulate the Y-Bus Matrix
...
for k = 1:nbranch
    ...
    % Handle tap ratios (if 0, it means standard nominal ratio of 1)
    if ratio == 0
        ratio = 1.0;
    end
    ...
    % Off-diagonal elements (accounting for transformer taps)
...
% Add bus shunt admittances (Gs + jBs) to diagonals
for i = 1:nbus
...
while iter < max_iter
    ...
    % Solve System using custom LU Decomposition
    % FIX 1: Changed output variable from 'x' to 'dx'
...
% FIX 2: Removed the duplicate/stray update code block that was here.
...
function [x, L, U] = calculate_lu(J, mismatch)
    ...
    % =======================================================
    % PART 2: Forward Substitution (Ly = b)
    % =======================================================
    y = zeros(n, 1);
    for i = 1:n
        sum_y = 0;
    ...
    % =======================================================
    % PART 3: Backward Substitution (Ux = y)
    % =======================================================
    x = zeros(n, 1);
    for i = n:-1:1
        sum_x = 0;
